...
{
	public function GetRefreshToken($client_id, $redirect_uri, $client_secret, $code) {	
		$url = 'https://accounts.google.com/o/oauth2/token';			
		
		$curlPost = 'client_id=' . $client_id . '&redirect_uri=' . $redirect_uri . '&client_secret=' . $client_secret . '&code='. $code . '&grant_type=authorization_code&access_type=offline&approval_prompt=force';
		$ch = curl_init();		
		...
		if($http_code != 200) 
			throw new Exception('Error : Failed to receieve refresh token');
			
		return $data;
	}
	
	public function GetAccessToken($client_id, $redirect_uri, $client_secret, $refresh_token) {	
		$url = 'https://accounts.google.com/o/oauth2/token';			
	...
	public function UpdateCalendarEvent($event_id, $calendar_id, $summary, $all_day, $event_time, $event_timezone, $access_token, $description = "") {
	...
	public function DeleteCalendarEvent($event_id, $calendar_id, $access_token) {
		...
		$data = json_decode(curl_exec($ch), true);
		$http_code = curl_getinfo($ch,CURLINFO_HTTP_CODE);
        //if($http_code != 204) throw new Exception('Error : Failed to delete event');
